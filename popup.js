...
function openBundlesPositioner() {
  ...
  function getCssSelector(el) {
    ...
    if (classAttr && typeof classAttr === 'string') {
      var classes = classAttr.split(' ').filter(function (c) { return c.trim() !== '' && c.indexOf(':') === -1; });
  ...
  function updateUI() {
    if (state === 'IDLE') {
      ...
      panel.innerHTML = '<h3 style="margin:0;font-size:16px;color:#008060;">&#10004; Script Generated</h3>' +
        '<p style="margin:0;margin-top:6px;font-size:12px;color:#6d7175;">1. Wrap this script inside <code style="background:#e3f1df;color:#008060;padding:2px 4px;border-radius:3px;">&lt;script&gt;&lt;/script&gt;</code> tags.</p>' +
        '<p style="margin:0;margin-top:4px;margin-bottom:8px;font-size:12px;color:#6d7175;">2. Paste it above the closing <code style="background:#e3f1df;color:#008060;padding:2px 4px;border-radius:3px;">&lt;/body&gt;</code> tag inside theme\'s <b>theme.liquid</b>.</p>' +
        '<textarea id="abp-output" style="width:calc(100% - 18px);font-family:SFMono-Regular,Consolas,Liberation Mono,Menlo,Courier,monospace;font-size:11px;padding:8px;border-radius:4px;border:1px solid #c9cccf;resize:vertical;background:#fff;color:#202223;" rows="9"></textarea>' +
        '<div style="display:flex;gap:8px;margin-top:5px;">' +
      ...
      document.getElementById('abp-btn-run').onclick = function () {
        try {
          var currentCode = document.getElementById('abp-output').value;
          eval(currentCode);
